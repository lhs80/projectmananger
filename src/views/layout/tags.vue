    ...
    <div class="tags-container">
        ...
        <div class="tags-box" ref="tagBox">
            <div class="tags-list" ref="tagsList" @mousewheel="hadelMousewheel" @mouseup="hadelMouseUp"
                ...
                <div ref="tagsPageOpened" class="tag-item" :class="{'is-active':nowTagValue==item.value}"
                     :name="item.value" @contextmenu.prevent="openMenu(item,$event)" v-for="(item,index) in GET_TAGLIST"
                     :key="index" @click="openUrl(item)">
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters('tags', ['GET_TAGWEL', 'GET_TAGLIST', 'GET_TAGS']),
			nowTagValue: function () {
				return setUrlPath(this.$route)
			},
			tagListNum: function () {
				return this.GET_TAGLIST.length != 0
			}
		},
		methods: {
			init() {
				...
				setTimeout(() => {
					this.refsTag.forEach((item, index) => {
						if (this.GET_TAGS.value === item.attributes.name.value) {
			...
			closeAllTags() {
				...
				this.$router.push({
					path: resolveUrlPath(this.GET_TAGWEL.value),
					query: this.GET_TAGWEL.query
				})
			},
			moveToView(tag) {
			...
			eachTag(tag) {
				for (var key in this.GET_TAGLIST) {
					if (this.GET_TAGLIST[key].value == tag.value) {
						return key
					}
				}
				return -1
			...
			closeTag(item) {
				...
				let tag
				this.$store.commit('tags/DEL_TAG', item);
				// if (item.value === this.tag.value) {
				tag = this.GET_TAGLIST[key == 0 ? key : key - 1]
				this.openUrl(tag)
				// }
			}
		}
