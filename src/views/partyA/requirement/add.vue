		...
		data() {
			return {
				...
				rules: {
					...
					joinPhone: [
						{pattern: phoneNo, message: '请输入加盟方联系电话', trigger: 'blur'}
					]
				}
			}
		},
		methods: {
			add(formName) {
				this.$refs[formName].validate(valid => {
					if (valid) {
						this.$store.dispatch('requirement/ADD_REQUIRE', this.requireInfo).then(res => {
